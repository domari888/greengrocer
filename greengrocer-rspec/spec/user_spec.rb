RSpec.describe User do
  describe "choose_product" do
    ...
    let(:pronpt_re_enter_msg){/#{products.first.id} ~ #{products.last.id} の番号から選んでください。/}
  ...
  describe ".dicide_quantity" do
    let(:user){User.new}
    shared_examples "@quantity_of_product の値が整数化した値と等しいこと" do
      it do
    ...
    context "1を入力したとき" do
      ...
      it_behaves_like "@quantity_of_product の値が整数化した値と等しいこと" 
    end
    ...
    let(:pronpt_re_enter_msg){/1個以上選んでください。/}
    shared_examples "再入力を促すこと" do
      it do
        allow(ARGF).to receive(:gets).and_return wrong_quantity_input, correct_quantity_input
        expect{user.dicide_quantity}.to output(pronpt_re_enter_msg).to_stdout
      end
    end
    context "0を入力したとき" do
